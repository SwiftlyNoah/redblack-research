...
observer with materially different beliefs. We then measure the object the
projection summarizes: the real game's public history tree has
$5.3\times10^{34}$ decision histories and exactly
$2{,}448{,}351{,}139{,}319{,}171{,}077{,}272{,}326{,}424{,}233{,}304$
perfect-recall infosets --- thirty orders of magnitude beyond the class
space --- so no method can engage it by tabulation. Finally we prove that
the beliefs the class forgets are not formless: under any strategy
profile, every reachable posterior factorizes over the class prior into
two per-seat tilts, giving the game a finite-dimensional belief state
whose shape is strategy-independent. Every count in this paper is
machine-verified against the shipped rules engine. Two companion papers
build on it: one solves the class abstraction exactly and measures what
the projection costs in play; the other uses the belief family to engage
the real game.
\end{abstract}
...
subject of two companion papers --- Paper~II solves the class abstraction
exactly and measures its cost; Paper~III engages the real game --- and
here we build the object those algorithms run on.
...
\begin{enumerate}\itemsep2pt
  ...
  \item Exact enumeration of the \emph{real} game: $5.3\times10^{34}$
        public decision histories and $2.4\times10^{33}$ perfect-recall
        infosets at $H=5$, counted exactly (not estimated) by a
        recurrence whose opponent-realizability test is
        Theorem~\ref{thm:support} itself, validated by explicit joint
        enumeration at small hand sizes (Section~\ref{sec:realsize}).
  \item The rank-one factorization of reachable beliefs: under any
        strategy profile the public posterior at a boundary is the class
        prior times two per-seat tilts (Theorem~\ref{thm:rankone}), with
        opponent-model-free own-side conditionals
        (Corollary~\ref{cor:modelfree}) --- the structural fact the
        companion papers' belief tracking and safe re-solving stand on
        (Section~\ref{sec:belieffamily}).
\end{enumerate}
...
% =========================================================================
\section{The Size of the Real Game}\label{sec:realsize}

Everything counted so far is a \emph{state}: a description of where the
machine is. But Red/Black is a game of hidden information, and the players
do not act on states --- they act on what they have \emph{seen}. A bid
changes no card and moves no counter, yet it is observed, remembered, and
(as Section~\ref{sec:forgets} makes precise) it changes what a rational
opponent believes. The decision points of the real game $G$ are therefore
\emph{histories}, not states, and this section counts them exactly. The
numbers justify, quantitatively, why every solution method in the companion
papers works with beliefs rather than with the history tree itself.
...
Two features distinguish this object from the states of
Section~\ref{sec:automaton}. First, bids enter it: the physical state
forgets a retracted bid chain the moment the round resolves, but the
history keeps every bid ever made. Second, flip \emph{order} enters it:
Lemma~\ref{lem:order} proved order irrelevant to the state, but the
sequence of deck choices is observed, and in $G$ an observation is part of
the decision point whether or not it moves a counter. Order irrelevance
collapses the machine; it does not collapse the information.
...
because ``$10^{34}$'' undersells the point that they are \emph{counted},
not estimated:
\begin{align*}
...
The opening round alone makes the state-versus-history gap vivid: the round
automaton of Section~\ref{sec:round} has $119{,}036$ lattice-collapsed
resolutions, but $1{,}888{,}256$ order-sensitive contest records ---
a factor of $15.9$ from flip order and deck choice alone, before bid chains
and cross-round composition multiply it to $10^{34}$.
...
$\binom{n-f}{\,r-p_r}$ free completions). The second is where this paper's
main theorem earns its keep a second time:
...
are \emph{exactly} the feasible set --- tight as well as sound --- this
test admits every realizable pair and no unrealizable one, so the
recurrence counts infosets exactly rather than bounding them. The
recurrence is validated against explicit joint enumeration of the full
game (both hands, both orderings, every deck choice) at $H=1$ ($42$ and
$40$, exact) and $H=2$ ($945{,}053$ and $945{,}049$, exact).
\end{proof}
...
does not carry must be reconstructed as \emph{belief} --- the subject of
...
boundary histories by $\sim\!10^{31}$ ($3.1\times10^{34}$ boundary
histories to $5{,}485$ reachable classes, through the intermediate
$27{,}990$ concrete boundary states of Table~\ref{tab:master}). The
infoset space of $G$ exceeds the class space by $\sim\!10^{29}$. And the
infoset space exceeds anything enumerable by any machine by more than ten
orders of magnitude. So there will be no solve of $G$ by tabulation, at
this hand size or anywhere near it: any method that engages $G$ itself
must represent histories through a summary that is small \emph{and}
strategically sufficient. The class is the right summary of public
\textbf{knowledge} (Theorem~\ref{thm:support}); what remains --- and what
Section~\ref{sec:forgets} takes up --- is whether anything tractable is a
sufficient summary of public \textbf{belief}.
...
omissions are not incidental --- they are the reason Paper~II's
decomposition needs care, and the raw material of Paper~III.
...
The two omissions above say what beliefs the class \emph{loses}. This
subsection proves the surprising positive counterpart: the beliefs
themselves --- over both seats' private situations jointly, under
\emph{any} strategy profile --- live in a family whose shape is a
property of the game, not of the profile.
...
the private statistic the companion papers solve over; the pending
material here is only its structure.)
...
deal, and by Lemma~\ref{lem:ainv} the deal enters the type pair only
through $(a_0,a_1)$, which is a deterministic function of the types
themselves --- so the deal coupling is absorbed into $\lambda_\kappa$ and
rides along every transition unchanged. Conditional on types, each seat's
...
Together with Section~\ref{sec:realsize}, this locates the game's true
sufficient statistic. The public class is the strategy-independent
\emph{knowledge}; the reachable \emph{belief} state is the class plus two
per-seat tilt vectors $(g_0,g_1)$ --- a finite-dimensional family, against
the $10^{34}$ raw histories that induce it. Strategy dependence enters
only through where in the family a profile lands, never through the
family's shape.
...
one representative profile, the solved one --- but they put a scale on
the qualitative claim of this section: the class prior at depth is nearly
orthogonal to the belief a competent observer actually holds. The
companion papers carry the consequences.
...
opponent's discard tendencies --- is belief rather than knowledge. What
this paper can still say about belief without committing to a strategy
model is Section~\ref{sec:belieffamily}: the reachable belief family has
a game-determined shape, $\lambda_\kappa\cdot g_0\cdot g_1$, and only the
tilts $(g_0,g_1)$ are anyone's model of anyone. The class is the complete
knowledge state; the class plus the tilts is the belief state; and the
companion papers are, respectively, what happens when a solver uses only
the first (Paper~II) and what it takes to responsibly use the second
(Paper~III).
...
This paper stops at the dynamics and the information structure. Two
companion papers carry the threads forward, and it is worth being precise
about which thread each one takes.

\paragraph{Paper II solves the class abstraction, exactly, and measures
the cost.} Section~\ref{sec:realsize} rules out engaging the history tree
directly, and Theorem~\ref{thm:support} makes the class the canonical
small summary --- so Paper~II decomposes the game at boundaries, re-derives
entry beliefs from the class prior $\lambda_\kappa$, and solves every one
of the $5{,}721$ subgame-bearing classes with CFR+. That belief re-derivation
is exactly the approximation Sections~\ref{sec:belieffamily}
and~\ref{sec:forgetsmeasured} put a scale on, and Paper~II's second half
measures what it costs in play: the abstraction's equilibrium is
exploitable in the real game in ways its own convergence meters cannot
see, including a converged action provably dominated in $G$.

\paragraph{Paper III uses the belief family this paper identified.}
Theorem~\ref{thm:rankone} says the reachable beliefs of $G$ form the
finite family $\lambda_\kappa\cdot g_0\cdot g_1$; Paper~III tracks the
tilts exactly with a public filter, re-solves each boundary under the
true posterior instead of the prior, and pursues a \emph{certified upper
bound} on real-game exploitability via safe re-solving.
Corollary~\ref{cor:modelfree} is load-bearing there: own-side
conditionals survive an arbitrarily wrong opponent model.
...
of Paper~II --- the second cost an early solver generation a measured
mispricing, and the first is the origin of everything Paper~III exists
to repair.
